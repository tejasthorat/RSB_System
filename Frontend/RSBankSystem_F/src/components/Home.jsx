...
const Home = () => {
  ...
  const [searchCircularNumber, setSearchCircularNumber] = useState("");
  ...
  const handleSearch = async () => {
    try {
      ...
      if (searchCircularNumber.trim() !== "") params.append("circularNumber", searchCircularNumber);
      ...
      const response = await axios.get(`http://localhost:8080/api/files/search?${params.toString()}`);
      const sortedFiles = response.data.sort(
        (a, b) => new Date(b.circularDate) - new Date(a.circularDate)
      );
      setFiles(sortedFiles);
    ...
    <div className="home-container">
      ...
      {showFilters && (
        <div className="filter-prompt">
          ...
          {/* Circular Number & Circular Type Dropdown (Below Line) */}
          <div className="circular-search-container">
            <input
              type="text"
              placeholder="Search by Circular Number"
              value={searchCircularNumber}
              onChange={(e) => setSearchCircularNumber(e.target.value)}
            />
